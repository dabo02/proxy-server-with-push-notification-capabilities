...
class UDPHandler(SocketServer.BaseRequestHandler):   
    ...
    def debugRegister(self):
        logging.info("*** REGISTRAR ***")
        logging.info("*****************")
        for key in registrar.keys():
            logging.info("%s -> %s" % (key,registrar[key][0]))
        logging.info("*****************")

    def sendPushNotification(self, payloadKeys):
        # token_hex = '94ded1b8278517de1676ed2ef4ad332fd522443d7893328b6055224c48311d98'
        token_hex = payloadKeys['token']
        payload = Payload(alert=payloadKeys["alert"], sound=payloadKeys["sound"], badge=payloadKeys["badge"])
        apns.gateway_server.send_notification(token_hex, payload)


    def storeAPNCredentials(self, creds):
        info = {'alert': 'Incoming Call', 'sound': 'Default', 'badge': 1}
        creds.update(info)
        tokens.update(creds)
        print tokens
        return
    ...
    def processRegister(self):
            ...
            if rx_contact.search(line) or rx_ccontact.search(line):
                ...
                if md:
                    ...
                    if rx_token.search(line):
                        ...
                        self.storeAPNCredentials(creds)
                else:
        ...
        logging.info("Expires= %d" % expires)
        registrar[fromm]=[contact, self.socket, self.client_address, validity]
        self.debugRegister()
        print registrar[fromm]
        print registrar

        # new_uri = tokens['uri']
        # new_uri = new_uri.replace(tokens['contact'], public_ip + ':5060')

        # self.sendResponse("200 0K")
        # data = string.join(self.data, "")
        # data.replace("Contact:<" + tokens['uri'], "Contact:<" + new_uri)
        # request_raw = list(data)
        # old_data = self.data[tokens['index']]
        # new_data = old_data.replace("Contact: <" + tokens['uri'], "Contact: <" + new_uri)
        # self.data[tokens['index']] = new_data
    ...
    def processInvite(self):
        ...
        if len(destination) > 0:
            ...
            #if firebase ios user on background:
                #get info from contact header
                #sendPushNotification
            #insert Record-Route
            # if tokens['token'] is not None:
                # self.sendPushNotification(tokens)
            data.insert(1, recordroute)
            text = string.join(data,"\r\n")
            # self.sendResponse("100 Trying")
            # print self.socket.getpeername()
            dest = tokens['contact'].split(":", 1)
            dest_port = int(dest[1])
            if self.client_address == ('63.131.240.90', 5060):
                self.socket.sendto(text, (dest[0], dest_port))

            else:
                self.socket.sendto(text , reg_addr)
            showtime()
            logging.info("<<< %s" % data[0])
            logging.info("---\n<< server send [%d]:\n%s\n---" % (len(text),text))
            # else:
        ...
        if len(destination) > 0:
            ...
            if registrar.has_key(destination) and self.checkValidity(destination):
                ...
                if self.client_address == ('63.131.240.90', 5060):
                    ...
                    self.socket.sendto(text , reg_addr)
                socket.sendto(text, (dest[0], dest_port))
                showtime()
                logging.info("<<< %s" % data[0])
                logging.info("---\n<< server send [%d]:\n%s\n---" % (len(text),text))    
            else:
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            text = string.join(data,"\r\n")
            self.socket.sendto(text, claddr)
            showtime()
            logging.info("<<< %s" % data[0])
            logging.info("---\n<< server send [%d]:\n%s\n---" % (len(text),text))
